...
const LessonProgram = ({ match }) => {
  ...
  return (
    <div className="lesson-shell">
      ...
      {step < 6 && <footer className={`lesson-navigation ${step === 1 || step === 2 ? 'lesson-navigation--expressions' : ''} ${step === 5 ? 'lesson-navigation--exercises' : ''}`}><button type="button" onClick={previous} disabled={step === 0}>← Înapoi</button><span>Pasul {step + 1} din {steps.length}</span><button type="button" className="primary" onClick={next} disabled={step === 5 && !canFinish}>{step === 5 ? (canFinish ? 'Finalizează lecția' : 'Răspunde la toate întrebările') : 'Continuă →'}</button></footer>}
    </div>
  );
};
